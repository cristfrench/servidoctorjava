...
<SCRIPT>
   ...
   function enviar(dato){
		...
		if (dato=='1'){
			window.opener.llegada1(1);
		}
		if (dato=='2'){
			window.opener.llegada1(2);
		}
				...
				<table class="tablag" align="center" cellpadding="0" cellspacing="0" border="0" width="550">
					<tr>
						...
						<bean:message key="5160"/><br><br>
	      				...
	      				String str = "";
	      				try{
	      				str=request.getParameter( "revisionX" );
							...
							String[] tokens = str.split(",");
		      				while(d<tokens.length){     						
	      						...
	      						while(c<revisionesVehiculo.size()){
		      						//Integer h1=new Integer(c);
	      							//String h=h1.toString();
	      							//String revisiones=tokens[d];
	      				...
	      				<%	}else{ %>
		      			<% 		int y=0;
		      					String quienes=request.getParameter("who").toString();
		      					Integer quien= new Integer(quienes);
		      					while(y<revisionesVehiculo.size()){
		      						if (y == quien.intValue()){
		      					%>
			      				<tr>
				      				<td class="view_item">&nbsp;<%=((RevisionesVehiculo) revisionesVehiculo.get(y)).getRveh_cod()%> - <%=((RevisionesVehiculo) revisionesVehiculo.get(y)).getRveh_actividad()%></td>
				      				<td class="view_item">&nbsp;<%=request.getParameter( "DefinitivaFechaControlBD0" )%></td>
				      				<td class="view_item">&nbsp;<%=request.getParameter( "DefinitivaDistanciaControlBD0" )%></td>
			      				</tr>
			      				<%	}
	      				...
	      				<br><br /><bean:message key="5130"/>
	      				<br><br><bean:message key="5131"/>
