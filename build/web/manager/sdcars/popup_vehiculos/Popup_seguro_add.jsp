...
<SCRIPT>
function actualizar(){
	document.forms[0].action="<%=request.getContextPath()%>/DatosVehiculo/Popup/add_seguro.do?change=1"
...
<html:form target="_self" action="/DatosVehiculo/Popup/add_seguro.do">
			...
			<TD>
				<table border="0" id="tab2" cellspacing="0" cellpadding="0" width="100%">
					<tr>
						<td colspan="5">
							<%
								if ( request.getSession().getAttribute( "borrar" ) != null ) {
							%>
							<SCRIPT language="JavaScript">actualiza_popup_abierto();self.close();</SCRIPT>
							<%
								}
							%>
					...
					<tr>
						<TD colspan="2" class="tdOverNameOption">
							...
							&nbsp;<%=request.getSession().getAttribute( "SEGUROSVEHICULOS" ) != null
						&& ( ( List ) request.getSession().getAttribute( "SEGUROSVEHICULOS" ) ).size() > 0 ? ( ( List ) request.getSession()
						.getAttribute( "SEGUROSVEHICULOS" ) ).size() + 1 : 1%>
						</TD>
